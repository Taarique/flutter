...
> This is only necessary for the first clone and run after that for every run `blutter.py` will automatically check for updates from the repo and if an update is available alongwith updating it'll replace occurances of `std`, thereby no need of running it everytime.
